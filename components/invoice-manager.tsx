...
const brandClients = {
  ...
  "What's Good Chicago": "What's Good Chicago",
}
...
export function InvoiceManager({
  ...
  const removeFromInvoice = async (brand: string, projectId: string) => {
    try {
      ...
      const { error } = await supabase.from("pmp_invoice_projects").delete().eq("project_id", projectId).eq("brand", brand)
  ...
  const clearInvoiceHistory = async (brand: string) => {
    ...
    try {
      // Clear from database
      const { error } = await supabase.from("pmp_exported_invoices").delete().eq("brand", brand)
  ...
  const getNextInvoiceNumber = async (brand: string): Promise<number> => {
    try {
      ...
      const { data: rpcResult, error: rpcError } = await supabase.rpc("pmp_get_next_invoice_number", {
        brand_name: brand,
      })
  ...
  const generateInvoicePDF = async (brand: string) => {
    ...
    try {
      ...
      const brandName = brand
        ...
        .replace("What's_Good_Chicago", "WHATS_GOOD_CHICAGO")
      ...
      // Save to database
      const { error } = await supabase.from("pmp_exported_invoices").insert([
        {
          brand,
      ...
      const { error: clearError } = await supabase.from("pmp_invoice_projects").delete().eq("brand", brand)
