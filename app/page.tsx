...
export default function ProjectManagementDashboard() {
  ...
  const loadProjects = async () => {
    try {
      ...
      const { data, error } = await supabase.from("pmp_projects").select("*").order("priority", { ascending: true })
  ...
  const processFiles = (files: File[]) => {
    ...
    if (currentCount >= maxFiles) {
      toast.error(`Maximum ${maxFiles} files allowed per project.`)
      return
    }
    ...
    if (files.length > remaining) {
      toast.error(`Only ${remaining} more file(s) allowed. ${files.length - remaining} file(s) skipped.`)
    }
    ...
    const newFileMap = new Map(pendingFiles)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Upload files to Supabase Storage (HD, no compression)
      const uploadedFiles: ProjectFile[] = []

      for (const formFile of formData.files) {
        const rawFile = pendingFiles.get(formFile.id)
        if (rawFile) {
          // New file that needs uploading
          const timestamp = Date.now()
          const safeName = formFile.name.replace(/[^a-zA-Z0-9._-]/g, "_")
          const storagePath = `pmp/${formData.brand}/${timestamp}_${safeName}`

          const { error: uploadError } = await supabase.storage
            .from("pmp-project-files")
          ...
          if (uploadError) {
            ...
            toast.error(`Failed to upload ${formFile.name}`)
            continue
          }
          ...
          // Existing file from editing (already has a real URL)
      ...
      if (editingProject) {
        const { error } = await supabase.from("pmp_projects").update(projectData).eq("id", editingProject.id)
        ...
        const { error } = await supabase.from("pmp_projects").insert([projectData])
      ...
      setFormData({
        ...
        description: "",
        deadline: undefined,
        priority: activeProjects.length + 1,
        files: [],
      })
  ...
  const handleStatusChange = async (id: string, status: Status) => {
    try {
      ...
      const { error } = await supabase.from("pmp_projects").update({ status }).eq("id", id)
  ...
  const handlePriceSubmit = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("pmp_projects").update({ status: "Completed" }).eq("id", priceDialog.project.id)
      ...
      const { error: invoiceError } = await supabase.from("pmp_invoice_projects").insert([
        {
      ...
      // Delete files from Supabase Storage to save space
      if (priceDialog.project.files && priceDialog.project.files.length > 0) {
        const storagePaths = priceDialog.project.files
          .filter((f: ProjectFile) => f.storagePath)
          .map((f: ProjectFile) => f.storagePath as string)
        ...
        // Clear files from the project record in db
        await supabase.from("pmp_projects").update({ files: [] }).eq("id", priceDialog.project.id)
      }
  ...
  const confirmDelete = async (id: string) => {
    try {
      const { error } = await supabase.from("pmp_projects").delete().eq("id", id)
  ...
  const confirmClearCompleted = async () => {
    ...
    if (clearDialog.brand && clearDialog.brand.includes('Delete "')) {
      ...
      if (projectToDelete) {
        try {
          const { error } = await supabase.from("pmp_projects").delete().eq("id", projectToDelete.id)
    ...
    try {
      ...
      const { error } = await supabase.from("pmp_projects").delete().eq("status", "Completed")
    ...
    if (active.id !== over?.id) {
      ...
      try {
        ...
        for (const update of updates) {
          await supabase.from("pmp_projects").update({ priority: update.priority }).eq("id", update.id)
        }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <Card className="hover:shadow-lg transition-shadow">
                <CardHeader className="pb-4">
                  ...
                  <CardDescription>{"Access all completed What's Good Chicago project files"}</CardDescription>
                </CardHeader>
                <CardContent>
                  <Button
                    onClick={() =>
                      window.open(
                        "https://drive.google.com/drive/folders/1I0fjR2Yi1XCAa_Lmmr7pmrXKGJOEwGpJ?usp=drive_link",
                        "_blank",
                      )
                    }
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="What's Good Chicago">{"What's Good Chicago"}</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500 mt-1">
                              Images, PDFs, Word docs, and text files up to 10MB each (max 8 files)
                            </p>
                            {formData.files.length > 0 && (
                              <p className="text-xs text-blue-600 mt-1 font-medium">
                                {formData.files.length}/8 files attached
                              </p>
                            )}
                          </div>
                        </div>
